...
install:
	...
	for a in bin/*; do cmp -s "$$a" "$$HOME/$$a" || ln --backup=t --relative -vs "$$a" "$$HOME/bin/." || exit $$?; done
	for a in home; do ( cd "$$a" && for b in *; do cmp -s "$$b" "$$HOME/.$$b" || ln --backup=t --relative -Tvs "$$b" "$$HOME/.$$b"|| exit $$?; done ) || exit $$?; done
	...
	[ 0 != "`id -u`" ] || { cd root && for a in *; do [ ! -d "$$a" ] && continue; find "$$a" ! -type d -print | while read -r b; do cmp -s "$$b" "/$$b" || { mkdir -vp "/`dirname "$$b"`" && ln --backup=t --relative -Tvs "$$b" "/$$b"; } || exit $$?; done || exit $$?; done; }
